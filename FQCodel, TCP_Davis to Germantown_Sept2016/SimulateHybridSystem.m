...
function [X, C, E, M, UStar] = SimulateHybridSystem(TCPCoDel, K, x_k, NSubintPerInt, c_k, AStar, BStar, Sigma_Ours_q1, Sigma_q0, ABar, Sigma_TCPCoDel_q1, beta, alpha_Gb, cStar_Gb, tStar_Gbps) 
[n, ~] = size(AStar);
...
for k = 1:K
    ...
    if k <= K-1 %Last mode is on [K-1, K), stored in M(K-1).
        if c_k >= cStar_Gb %mode q1
            M(k) = 1;
            if TCPCoDel==1 %mode q1, TCP-CoDel controller
                A = ABar; Sigma = Sigma_TCPCoDel_q1; B = zeros(n,1); UStar(k) = 0;
            elseif TCPCoDel==0 %mode q1, Our controller
                A = AStar; Sigma = Sigma_Ours_q1; B = BStar; UStar(k) = GetOptimalControl(AStar, BStar, k, X);
            end
        else %mode q0
            M(k) = 0; 
